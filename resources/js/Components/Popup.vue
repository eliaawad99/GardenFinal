...
<template>
    ...
    >
        <div
            class="pl-12 pt-11 pb-10 pr-16 bg-custom-yellow skew-y-1 rounded-tl-[25px] rounded-tr-[40px] rounded-br-[15px] rounded-bl-[30px]"
        >
            <div class="flex">
                <img
                    ...
                    class="custom-cursor"
                />
                <p class="text-custom-green text-3xl ml-6">
            ...
            <form @submit.prevent="submit">
                <div class="flex pl-9 pt-10">
                    <div class="pr-11">
                        <div
                            class="bg-input-color h-28 rounded skew-x-3 w-[400px] relative"
                        >
                            <label
                                for="image"
                        ...
                        <TextInput
                            ...
                            required
                            autofocus
                            placeholder="Plant Child Name"
                        />
                        <DropdownInput
                            v-model="form.species"
                            :placeholder="selectPlaceholderSpecies"
                        ...
                        <div
                            class="flex bg-input-color skew-x-12 w-[400px] h-[48px] mt-6 border-0 rounded justify-between items-center pl-6 text-custom-gold"
                        >
                            <label for="date">Date Planted</label>
                            <input
                                class="bg-transparent border-none focus:ring-transparent"
                                id="date"
                                type="date"
                        ...
                        <DropdownInput
                            ...
                            :placeholder="selectPlaceholderSoil"
                            :options="selectOptionsSoil"
                        ></DropdownInput>
                    </div>
                    <div>
                        <DropdownInput
                            v-model="form.drainage"
                            :placeholder="selectPlaceholderDrainage"
                            :options="selectOptionsDrainage"
                            class="!mt-0"
                        ></DropdownInput>
                        <DropdownInput
                            v-model="form.fertilizer"
                            :placeholder="selectPlaceholderFertilizer"
                            :options="selectOptionsFertilizer"
                        ></DropdownInput>
                        <RateInput type="Sunlight">
                            <div class="flex gap-1">
                                <SunIcon
                                    v-for="n in 5"
                                    :key="n"
                                    :color="isSun(n) ? 'green' : '#DED295'"
                                    @click="selectSunlight(n)"
                                />
                            </div>
                        </RateInput>
                        <DropdownInput
                            v-model="form.humidity"
                            :placeholder="selectPlaceholderHumidity"
                            :options="selectOptionsHumidity"
                        ></DropdownInput>
                        <textarea
                            v-model="form.notes"
                            class="bg-input-color skew-x-3 w-[400px] h-[192px] mt-6 border-0 rounded placeholder:text-custom-gold focus:ring-transparent overflow-auto resize-none"
                            placeholder="Notes"
                        ></textarea>
                    </div>
                </div>
